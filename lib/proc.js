...
function run(key, proc, emitter) {
    var child = prog.exec(proc.command, {
        env: proc.env,
        cwd: proc.cwd
    });
...
function start(procs, requirements, envs, portarg, emitter, cwd) {
    var j = 0;
    var k = 0;
    var port = parseInt(portarg);

    if (port < 1024) return cons.Error('Only Proxies Can Bind to Privileged Ports - ' + 'Try \'sudo nf start -x %s\'', port);
    ...
    for (var key in requirements) {
        ...
        for (i = 0; i < n; i++) {
            ...
            var p = {
                ...
                env: merge(merge({ PORT: port + j + k * 100, CWD: cwd }, process.env), envs),
                cwd: cwd
            };
